
export const sidebar = {
    container: 'lg:w-[30%] w-full h-full bg-slate-200',

    header: 'w-full h-75 bg-[#20c7ff] rounded-b-[30%] shadow-gray-400 shadow-lg flex flex-col gap-2.5 justify-center px-5',

    title: 'text-white font-bold text-[25px]',

    userRow: 'w-full flex justify-between items-center',

    userName: "text-gray-800 font-bold text-[20px]",

    profileImageWrapper: "w-12 h-12 mt-2.5 rounded-full overflow-hidden flex justify-center items-center shadow-gray-500 shadow-lg",

    profileImage: "h-full object-cover",

    searchOffWrapper: "w-12 h-12 bg-white rounded-full overflow-hidden flex justify-center items-center mt-2.5 shadow-gray-500 shadow-lg cursor-pointer",

    searchOnWrapper: 'w-full h-12 bg-white shadow-gray-500 shadow-lg flex items-center gap-2.5 mt-2.5 rounded-full overflow-hidden p-5',

    logout: 'w-15 h-15 bg-[#20c7ff] text-gray-700 rounded-full overflow-hidden flex justify-center items-center mt-2.5 shadow-gray-500 shadow-lg cursor-pointer fixed bottom-5 left-5',
}